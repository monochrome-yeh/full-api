...
namespace common\components\monochrome;

use Yii;

class zipCodeTW extends \yii\base\Component
{

    private $code = ["基隆市"=>["仁愛區"=>"200",
...
"五峰鄉"=>"311","橫山鄉"=>"312","尖石鄉"=>"313","北埔鄉"=>"314","峨嵋鄉"=>"315"],"桃園市"=>["中壢市"=>"320","平鎮市"=>"324","龍潭鄉"=>"325","楊梅鎮"=>"326","新屋鄉"=>"327","觀音鄉"=>"328","桃園市"=>"330","龜山鄉"=>"333","八德市"=>"334","大溪鎮"=>"335","復興鄉"=>"336","大園鄉"=>"337","蘆竹鄉"=>"338"],
...
"西嶼鄉"=>"881","望安鄉"=>"882","七美鄉"=>"883","白沙鄉"=>"884","湖西鄉"=>"885"],"南海諸島"=>["東沙"=>"817","南沙"=>"819"]];


	public function getCounty($zip = null) {
		if ($zip == null) {
			return array_keys($this->code);
		}
		else {
			foreach ($this->code as $key => $value) {
				if(in_array((integer)$zip, $value)) {
					return $key;
				}
			}
		}
	...
	public function getDistrict($zip = null) {
			foreach ($this->code as $key => $value) {
				if(in_array((integer)$zip, $value)) {
					return array_flip($value)[$zip];
				}
			}
	}	
	...
	public function getAll() {
		$data = [];
		foreach ($this->code as $key => $value) {
			foreach ($value as $district => $zip) {
				$data[] = ['county' => $key, 'zip' => $zip, 'district' => $district];
			}
		}
		return $data;
	}
